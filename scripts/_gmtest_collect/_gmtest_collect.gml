function _gmtest_collect() {
	...
	var script_index = global.GMTEST.SCRIPT_INDEX_START
	var test_ids = []
	
	while script_index < global.GMTEST.SCRIPT_INDEX_END {
		var is_script_present = script_exists(script_index)
		if is_script_present {
			var script_name = script_get_name(script_index)
			if string_pos(global.GMTEST.NAME_PREFIX, script_name) == 1 {
